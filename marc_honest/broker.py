...
def ingest(input_fp: Path) -> pd.DataFrame:
    ...
    metadata_columns = [col for col in df.columns if col not in tube_columns + box_columns]

    # Melt the DataFrame to have one row per tube/box (only for existing tube columns)
    df_long = df.melt(id_vars=metadata_columns, 
                      value_vars=tube_columns,
                      var_name="Tube Type", 
                      value_name="Tube Barcode")

    # Extract the tube type (a, b, or c)
    df_long["Tube Type"] = df_long["Tube Type"].str.replace("Tube Barcode ", "", regex=False)

    # Melt the box-name-position columns as well (only for existing box columns)
    df_boxes = df.melt(id_vars=metadata_columns, 
                       value_vars=box_columns,
                       var_name="Box Type", 
                       value_name="Box-name_position")

    # Extract the corresponding box type (a, b, or c)
    df_boxes["Box Type"] = df_boxes["Box Type"].str.replace("Box-name_position ", "", regex=False)

    # Merge the two melted dataframes on sample metadata and matching Tube/Box types
    df_merged = pd.merge(df_long, df_boxes, left_on=metadata_columns + ["Tube Type"], 
                         right_on=metadata_columns + ["Box Type"], how="left")
...
def load(df: pd.DataFrame, session: Session) -> pd.DataFrame:
    for i, r in df.iterrows():
        ...
        if not session.query(Subject).filter(Subject.mrn == mrn).first():
            ...
            session.add(subject)
        subject_id = int(session.query(Subject).filter(Subject.mrn == mrn).first().subject_id)

        if not session.query(Specimen).filter(Specimen.specimen_barcode == specimen_barcode).first():
            specimen = Specimen(specimen_barcode=specimen_barcode)
            session.add(specimen)
        specimen_id = int(session.query(Specimen).filter(Specimen.specimen_barcode == specimen_barcode).first().specimen_id)
